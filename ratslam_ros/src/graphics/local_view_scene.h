...
{
public:
  LocalViewScene(ptree & settings, LocalViewMatch *in_vt):device(NULL),scene(NULL),view_template_scene(NULL),driver(NULL)
  {
	...
	local_view_vtk_show(vtm->view_rgb,(const double*)&vtm->templates[vtm->current_vt].data[0],(const double*)&vtm->current_view[0]);
...
//    device = irr::createDevice(irr::video::EDT_OPENGL, irr::core::dimension2d<irr::u32>(vt_window_width, vt_window_height), 32, false, false, false);
...
//    view_template_scene = scene->createNewSceneManager(false);
  ...
  void draw_all_vtk()
  {
//	  ROS_FATAL_STREAM("local_view_vtk_show before.");
	  local_view_vtk_show(vtm->view_rgb,(const double*)&vtm->templates[vtm->current_vt].data[0],(const double*)&vtm->current_view[0]);
//	  ROS_FATAL_STREAM("local_view_vtk_show is done.");
  }
  void draw_all()
  {
